...
def is_geeksforgeeks_url(url):
    ...
    return parsed_url.netloc == 'www.geeksforgeeks.org'

# Function to get metadata
def get_metadata(url):
    if not is_geeksforgeeks_url(url):
        st.warning("Please enter a valid GeeksforGeeks URL (https://www.geeksforgeeks.org/).")
        return None
    ...
    try:
        ...
        return {
            ...
            "description": [meta.get("content", "").strip() for meta in meta_tags if "description" in str(meta).lower()],
            "text_content": article_text_content.strip(),
...
if url:
    ...
    if is_geeksforgeeks_url(url):
        if metadata:
            ...
            keywords_list = [keyword.strip() for keyword in metadata['keywords'][0].split(',')] if metadata[
                'keywords'] else []
            ...
            with tab3:
                ...
                fig = px.bar(keyword_density_df, x='Keyword', y='Density', title='Keyword Density Distribution',
                             height=500,
                             width=800)
                st.plotly_chart(fig)
